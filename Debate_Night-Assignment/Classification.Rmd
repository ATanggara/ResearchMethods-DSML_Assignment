...
```{r}
data_train$isbot <- as.character(data_train$isbot)
data_train$isbot[which(data_train$isbot == T)] <- "bot"
data_train$isbot[which(data_train$isbot == F)] <- "human"

data_train$isbot <- factor(data_train$isbot)
...
model_weights <- ifelse(data_train$isbot == TRUE,
                        (1/table(data_train$isbot)[1]) * 0.5,
                        (1/table(data_train$isbot)[2]) * 0.5)
...
```{r}
data_test$isbot <- as.character(data_test$isbot)
data_test$isbot[which(data_test$isbot == T)] <- "bot"
data_test$isbot[which(data_test$isbot == F)] <- "human"
data_test$isbot <- as.factor(data_test$isbot)
