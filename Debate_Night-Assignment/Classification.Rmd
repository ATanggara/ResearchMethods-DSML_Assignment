...
# remove rows with no numeric value assigned
toDel <- which(bs == "deleted")
bs <- bs[-toDel]
toDel <- which(bs == "suspended")
bs <- bs[-toDel]
toDel <- which(bs == "protected")
bs <- bs[-toDel]
...
cor(numdata)
```
'botscore' highest correlation is with 'friendsCount', 'followersCount', and 'listedCount'.
...
```{r}
svmfit <- svm(isbot ~ ., data = data_train, kernel = "sigmoid", cost = 0.2, scale = FALSE)
svmfit
```
