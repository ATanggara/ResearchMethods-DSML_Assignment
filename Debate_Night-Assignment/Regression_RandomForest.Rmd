...
normalized <- cbind(ntarget, dataset$verified, dataset$location.objectType, normalized)
# normalized <- data.frame(normalized)
colnames(normalized)[1] <- "botscore"
colnames(normalized)[3] <- "location.objectType"
colnames(normalized)[2] <- "verified"

normalized$verified <- as.factor(normalized$verified)
normalized$location.objectType <- as.factor(normalized$location.objectType)
...
trainingRowIndex <- sample(1:nrow(data_df), 0.2*nrow(data_df)) # row indices
