...
data_df[which(data_df$followersCount == 0), ][,c('followersCount', 'friendsCount')]
...
```{r}
large_psi_nofollow = data_train[which(data_train$psi>12 & data_train$followersCount==0), ]
psi_aug = large_psi_nofollow[, c('psi', 'followersCount', 'friendsCount')]
psi_aug

data_train[which(data_train$psi>12 & data_train$followersCount==0), ] <- 1

data_train[which(data_train$psi>12), ][, c('psi', 'followersCount', 'friendsCount')]
...
```{r}
# datapoints with high psi value
psi_comp = c('friendsCount', 'followersCount', 'psi')
large_psi = data_train[which(data_train$psi > 12), ]
large_psi[, psi_comp]
```
...
model_lm_psi <- train(botscore ~ psi + influence_percentile + verified, data = data_train,
...
head(data.frame(predicted = model_lm_psi_pred, 
                observed = data_test$botscore, row.names = NULL), n=10)
```
